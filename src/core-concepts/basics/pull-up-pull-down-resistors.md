...
Imagine you connect a button directly to a GPIO pin on your Pico. When the button is pressed, it connects the pin to ground (0V). When released, you might expect the pin to read as HIGH, but it doesn't work that way.  Instead, the pin is not actively driven to any defined voltage. It's floating in an undefined state, acting like an antenna that picks up electrical noise from nearby circuits, your hand, or even radio waves in the air.
...
By the way, you can also connect the button the other way around; connecting one side to 3.3V instead of ground. However, you'll face the same issue. When the button is pressed, it connects to the HIGH state. When released, you might expect it to go Low, but instead it's in a floating state again.
...
When the button is not pressed, current flows through the resistor to the GPIO pin, holding it at 3.3V (HIGH). When you press the button, you create a direct path to ground. The button provides a much lower-impedance path to ground than the pull-up resistor, forcing the GPIO pin voltage to LOW.
...
The Raspberry Pi Pico provides selectable internal pull-up or pull-down resistors on every GPIO pin, with only one enabled at a time. These internal resistors are relatively weak and intended for logic-level inputs such as buttons. You don't need to add external resistors for basic button inputs; you can enable them in software.
